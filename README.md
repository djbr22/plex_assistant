...
Plex Assistant is a Home Assistant component to allow Google Assistant, Home Assistant's conversation integration, and more to cast Plex media to Google devices and Plex clients. You can also use this component with anything that can make a service call to HA as well (see the example service call below).
...
You can either use IFTTT or DialogFlow to trigger Plex Assistant with Google Assistant. IFTTT is the easiest and recomended way to set this up. DialogFlow is a bit more involved and has some major quirks, but has support for more languages.
...
**This does not enable Google Assistant support, this is only for HA's built in voice assistant**.

By default the phrases `Tell Plex to {command}` & `{command} with Plex` work with the HA voice assistant ( [if you've enabled it](https://www.home-assistant.io/integrations/conversation/) ). This will also work with any of the languages from the table above, but you will need to add your own trigger phrases in the language of your choice. Below is an example of how to add custom phrases.

```yaml
conversation:
  intents:
    Plex:
     - "I ask the media Gods to {command}"
     - "{command} oh great lords of Plextopia"
```
